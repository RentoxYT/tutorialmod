...
import rentoxyt.tutorialmod.blocks.entity.CustomChestBlockEntity;

public class CustomChestMenu extends AbstractContainerMenu {
    public final CustomChestBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public CustomChestMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public CustomChestMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.CUSTOM_CHEST_MENU.get(), id);
        checkContainerSize(inv, 27);
        blockEntity = (CustomChestBlockEntity) entity;
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            ...
            for (int r = 0; r < 3; r++) {
                for(int c = 0; c < 9; c++) {
                    this.addSlot(new SlotItemHandler(handler, c + r * 9, 8 + c *18, 17 + r * 18));
                }
            }
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.CUSTOM_CHEST.get());
    }
